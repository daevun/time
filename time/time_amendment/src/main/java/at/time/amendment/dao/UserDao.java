...
import at.time.amendment.database.DatabaseManager;
...
public class UserDao {
	...
	public User getByOid(final String oid) {
		...
		try (Connection connection = dbManager.getConnection();
				PreparedStatement ps = connection.prepareStatement("SELECT * FROM amendment.user WHERE OID = ?")) {
			ps.setString(1, oid);
	...
	public void saveUser(final User user) {
		try (Connection connection = dbManager.getConnection();
				PreparedStatement ps = connection
						.prepareStatement("INSERT INTO amendment.user (oid, name, surname) values(?, ?, ?)")) {
